...
/**
 * This class provides the basic interface and implementation for the Travelling Salesman Problem faced in this software 
 */
public abstract class TSPSolver {
	...
	/**
	 * Gives back the best solution found so far during the computing of the TSP problem
	...
	/**
	 * Indicates if the time limit was reached during the last atempt to compute the best TSP path 
	 * @return true if the time limit was reached during the lat computation of the best path, false otherwise.
	 */
	 ...
	 * If the computation time exceeds the time limit, the computation will stop and the method timeLimitReached of this
	 ...
	 * @param coutVus la somme des couts des arcs du chemin passant par tous les sommets de vus + la somme des duree des
	 * sommets de vus
	 * @param tpsDebut : moment ou la resolution a commence
	 * @param tpsLimite : limite de temps pour la resolution
	 */	
	 private void branchAndBound(int sommetCrt, int coutVus, long tpsDebut){
		 /** The computation has been going on for too long, the algorithm stops here */
		 if (System.currentTimeMillis() - tpsDebut > TIMELIMIT){
			 timeLimitReached = true;
			 return;
		 }
		 if (unseenIntersections.size() == 0){ //All intersections have been visited
			 ...
			 if (coutVus < bestSolutionCost){ // We found a new better solution !
				 seenIntersections.toArray(bestSolution); //We store the solution (the order of visiting)
	        ...
	        while (it.hasNext()){
	        	...
	        	/** Reecursive call */
	        	branchAndBound(prochainSommet, coutVus + costs[sommetCrt][prochainSommet] + stayTime[prochainSommet], tpsDebut);
